...
class CustomerLoginUseCase @Inject constructor(private val authRepository: AuthRepository) {


    operator fun invoke(customerLoginDTO: CustomerLoginDTO): Flow<Resource<CustomerRegistrationResponse>> =
        flow {
            try {
                emit(Resource.Loading())
                val response = authRepository.login(customerLoginDTO)
