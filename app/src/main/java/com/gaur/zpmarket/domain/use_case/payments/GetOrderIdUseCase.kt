...
class GetOrderIdUseCase @Inject constructor(private val paymentRepository: RazorPayPaymentRepository) {


    operator fun invoke(amount: Int): Flow<Resource<OrderIdDTO>> =
        flow {
            try {
                emit(Resource.Loading())
                val response = paymentRepository.getOrderId(amount)
