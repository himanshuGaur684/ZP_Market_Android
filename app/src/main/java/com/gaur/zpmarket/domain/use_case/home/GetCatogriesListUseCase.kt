...
class GetCatogriesListUseCase @Inject constructor(private val customerHomeRepository: CustomerHomeRepository) {

    operator fun invoke(): Flow<Resource<CategoryDTO>> =
        flow {
            try {
                emit(Resource.Loading())
                val response = customerHomeRepository.getCategoriesList()
